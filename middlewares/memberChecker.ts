...
const memberChecker = async (req: Request, res: Response, next: NextFunction) => {
	console.log('🔐memberChecker-', req.userId, req.params);
	...
	try {
		...
		if (userId && foundProject) {
			...
			console.log('🔐memberChecker-chk:', projectURL, 'member:', chkMembers); // test
			if (chkMembers.includes(userId)) {
				console.log('🔐memberChecker-result:', userId, 'is member in', projectURL);
				// 실제 요청으로 넘어감
				console.log('🔐memberChecker-go next function!!\n');
				next();
			} else {
